<template>
  <!-- 拆表 -->
  <div class="MeterReadingOfYearSummary">
    ...
    <div v-if="indexShow" :class="{ 'company-content': tableQuery.statisticsWay == 1 ||  tableQuery.statisticsWay == 2}">
          ...
          >
            <el-form-item class="form-left">
              <el-form-item label="统计方式：">
                <el-select
                  v-model="tableQuery.statisticsWay"
                  placeholder="请选择"
                  ref="select1"  @change="selectChanged"
                >
                  <el-option
              ...
              <el-form-item v-if="showStatisticsDialog" label="抄表员">
                <el-input
                  v-model="tableQuery.readingStaff"
                  size="mini"
                  clearable
                  @keydown.enter.native="search"
                ></el-input>
...
export default {
  name: "MeterReadingOfYearSummary",
  components: {
    autoTree
  },
  data() {
    return {
      ...
      //面包屑
      crumbsData: {
        titleList: [{ title: "抄表水量报表" }, { title: "水量年汇总" }],
      },
      tableShow: false,
      maxHeight: 0,
      waterTypeNo: "",
  ...
  mounted() {
    ...
    //this.tableQuery.accountPeriod = new Date().getFullYear();
    //  var myDate = new Date();
    //     this.currentYear = myDate.getFullYear(); //获取当前年份(2位)
    //     console.log(this.currentYear)
    //     this.tableQuery.accountPeriod = this.currentYear
    // this.init();
    //data 里面定义一个axHeight 用于设置table的高度，增加一个v-if事件用于渲染吗，监听maxHeight 的变化控制v-if的变量

    // this.init()
    this.common.changeTable(this, ".MeterReadingOfYearSummary", [
      ".MeterReadingOfYearSummary .toolbar",
      ".MeterReadingOfYearSummary .block",
      ".MeterReadingOfYearSummary #crumbs",
    ]);
  },
  methods: {
        enterEvents(e){
    ...
    init() {
      ...
      // this.tableQuery.statisticsWay = parseInt(this.tableQuery.statisticsWay)
      //  this.tableQuery.accountPeriod = parseInt(this.tableQuery.accountPeriod)
      let _this = this;
      let params = {
        busicode: "MrYearStatisticsReport",
    ...
    Print() {
      let arr = [];
      let params = {
        busicode: "MrYearStatisticsReportPrint",
    ...
    exportExcel() {
      let params = {
        busicode: "MrYearStatisticsReportExport",
        data: this.tableQuery,
