<template>
  <!-- 实收统计表 -->
  <div class="ChargeReportOfRealReceivablesStatistics">
    ...
    <div v-if="indexShow" class="index-page">
      <div class="toolbar">
       ...
       <!-- <el-row>
          ...
          <!--</el-col>
          <el-col :span="6">-->
            <!-- <el-form-item label='缴费时间：' class="dateAndTime">
          ...
          <el-col :span="6">-->
            <el-form-item label="营业区域：" class="item">
              <el-cascader ref="cascader" clearable v-model="tableQuery.businessArea" :options="businessArea"
                           :show-all-levels="false" :props="tprops" class="shouldWater">
              </el-cascader>
            </el-form-item>
        <!--  </el-col>
        </el-row>-->
        <!-- <el-row>
          ...
          <!-- <el-col :span="3"> -->
            ...
            <el-form-item label="缴费方式：">
							<el-select ref="select2" clearable v-model="tableQuery.payWay" placeholder>
						...
						<el-form-item label="收费员：">
							<el-select filterable  ref="select3" clearable v-model="tableQuery.chargeStaffName" placeholder="全部">
      ...
      <div class="kl-table" :style="{height: maxHeight + 'px'}">
        <el-table  v-if="tableShow" highlight-current-row :max-height="maxHeight" stripe border show-summary :summary-method='getTotal'  :data="tableData.list" class="ChargeReportOfRealReceivablesStatistics-table"  ref="eltableCurrentRow">
          ...
          <el-table-column v-if="showChargeStaffDialog" prop="chargeStaffName" label="收费员" min-width="120">
          </el-table-column>
					<el-table-column v-if="showPayChannelDialog" prop="payChannel" label="收费渠道" min-width="120">
          </el-table-column>
          <el-table-column v-if="showPayWayDialog" prop="payWay" label="收费方式" min-width="120">
          </el-table-column>
				<!-- <template v-if="showChargeStaffDialog">
          <el-table-column v-if="showChargeStaffDialog" prop="chargeStaffName" label="收费员" min-width="120">
          </el-table-column>
					<el-table-column v-if="showPayChannelDialog" prop="payChannel" label="收费渠道" min-width="120">
          </el-table-column>
          <el-table-column v-if="showPayWayDialog" prop="payWay" label="收费方式" min-width="120">
				...
				<template v-else-if="showPayChannelDialog">
					<el-table-column v-if="showPayChannelDialog" prop="payChannel" label="收费渠道" min-width="120">
          </el-table-column>
					<el-table-column v-if="showPayChannelDialog" prop="chargeStaffName" label="收费员" min-width="120">
          </el-table-column>
          <el-table-column v-if="showPayChannelDialog" prop="payWay" label="收费方式" min-width="120">
          </el-table-column>
				</template>
				<template v-else-if="showPayWayDialog">
					<el-table-column v-if="showPayWayDialog" prop="payWay" label="收费方式" min-width="120">
          </el-table-column>
					<el-table-column v-if="showPayWayDialog" prop="chargeStaffName" label="收费员" min-width="120">
          </el-table-column>
					<el-table-column v-if="showPayWayDialog" prop="payChannel" label="收费渠道" min-width="120">
          </el-table-column>
    		</template> -->

          <el-table-column prop="realityWater" min-width="120" label="合计水量"   show-overflow-tooltip align="right">
          </el-table-column>
					<el-table-column prop="processedMoney" min-width="120" label="实收金额" show-overflow-tooltip align="right">
          </el-table-column>
...
<!--          <el-table-column prop="baseWaterTotal" min-width="120" label="基本水费" show-overflow-tooltip  align="right" >
          ...
          <el-table-column prop="ladderJ6" min-width="110" label="六阶金额" show-overflow-tooltip  align="right" >
					...
					<el-table-column :prop="item.costAttr" :label="item.costName" min-width="150" align="right" v-for="(item,index) in costNameList" :key="'info_'+index" show-overflow-tooltip  >
          ...
          <!-- <el-table-column prop="priorPeriodBalance" min-width="120" label="上期预存" show-overflow-tooltip align="right">
          </el-table-column>
          <el-table-column prop="transactionMoney" min-width="120" label="本期预存" show-overflow-tooltip align="right">
          </el-table-column>
          <el-table-column prop="currendPeriodBalance" min-width="120" label="预存余额" show-overflow-tooltip align="right">
          </el-table-column> -->
        </el-table>
...
<script>
export default {
  name: "ChargeReportOfRealReceivablesStatistics",
  components: {
  ...
  data() {
    return {
      ...
      tableQuery: {
        ...
        // sort:'',
        // startAccountPeriod: this.common.date('month'),
      ...
      dictDataOptions:[],//水表分类下拉框
			//数据字典存值
      dictionaryData: {},
			chargingStaffList: [],//收费员下拉数据
      //面包屑
      crumbsData: {
        titleList: [{ title: "收费管理报表" }, { title: "实收统计表" }]
      },
      tableShow: false,
      maxHeight: 0,
      selectList: [],
  ...
  mounted() {
    ...
    this.common.changeTable(this, ".ChargeReportOfRealReceivablesStatistics .index-page", [
      ".ChargeReportOfRealReceivablesStatistics .toolbar",
      ".ChargeReportOfRealReceivablesStatistics .block",
    ]);
  ...
  created() {
    ...
    let month = (time.getMonth() + 1) > 9 ? (time.getMonth() + 1).toString() : '0' + (time.getMonth() + 1)
    let lastDay = new Date(time.getFullYear(),month,0).getDate();
    this.chargeTime = [time.getFullYear() +"-"+ month+'-01', time.getFullYear() +"-"+ month +"-"+ lastDay];
    this.tableQuery.startChargeTime = time.getFullYear() +"-"+ month+'-01';
    this.tableQuery.endChargeTime = time.getFullYear() +"-"+ month +"-"+ lastDay;
  },
  computed: {
    showbookNos() {
  ...
  methods: {
   ...
   /*   if (this.tableQuery.less === '<'){
      ...
      let params = {
        busicode: "ProceedsSummaryReport",
    ...
    exportExcel() {
      ...
      // })
      let params = {
        "busicode": "ProceedsSummaryReportExport",
        "data": this.tableQuery,
  ...
  watch: {
    ...
    "tableQuery.statisticsWay": {
      handler: function (newval, oldval) {
        ...
        if (newval == "1") {
          //收费员
          this.showChargeStaffDialog = true;
          this.showPayChannelDialog = false;
					this.showPayWayDialog = false;
        } else if (newval == "2") {
          //收费渠道
					this.showChargeStaffDialog = false;
					this.showPayChannelDialog = true;
          this.showPayWayDialog = false;
        } else if (newval == "3") {
          //收费方式
          this.showChargeStaffDialog = false;
...
.ChargeReportOfRealReceivablesStatistics {
  width: 100%;
  height: 100%;
  .index-page {
