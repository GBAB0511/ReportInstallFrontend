  ...
  <div class="CheapStrategyEdit">
      ...
      <el-form class="formBill-Two" :inline="true" size="mini" ref="CheapStrategyEditForm" :rules="rules" :model="formData" label-width="100px">
        <el-form-item label="策略名称：" prop="strategyName">
          <el-input oninput ="value=value.replace(/\ +/g,'').replace(/[\r\n]/g,'')"  v-model="formData.strategyName" placeholder="请输入策略名称"></el-input>
        </el-form-item>
      ...
      <div class="toolbar" style="clear: both;height: 60px;">
        <div style="float: left;clear: both; width: 100%;" class="info">
          <div class="toolbar-line"></div>优惠明细
          <el-button size="mini" icon="el-icon-plus" type="primary" @click="addRow" style="float: right;">添加行</el-button>
        </div>
      </div>
      <el-table stripe border v-if="tableShow" :max-height="maxHeight" :data="formData.prefStrategyList" class="wuserInfo-table">
        ...
        <el-table-column prop="minRange" min-width="100" label="最小生效范围">
          <template slot-scope="scope">
            <el-input oninput ="value=value.replace(/\ +/g,'').replace(/[\r\n]/g,'')"  v-model="scope.row.minRange" type="number" maxlength="10" clearable placeholder="请输入最小生效范围">
            </el-input>
          </template>
        </el-table-column>
        <el-table-column prop="maxRange" min-width="100" label="最大生效范围">
          <template slot-scope="scope">
            <el-input v-model="scope.row.maxRange" type="number" oninput="if(value.length>10)value=value.slice(0,10)" clearable placeholder="请输入最大生效范围">
            </el-input>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="prefValue" min-width="100" label="优惠值">
          <template slot-scope="scope">
            <el-input oninput ="value=value.replace(/\ +/g,'').replace(/[\r\n]/g,'')"  v-model="scope.row.prefValue" type="number" maxlength="14" clearable step="0.0001" placeholder="请输入优惠值">
            </el-input>
          </template>
        </el-table-column>
  ...
  methods: {
    ...
    submitForm(type) {
      ...
      let params = {};
        ...
        let arr = params.data.prefStrategyList;
        for(var i=0;i<arr.length;i++){
          var item = arr[i];
          if(Number(item.minRange) >Number(item.maxRange)){
            _this.$message({
              showClose: true,
              message: "最小生效范围不能大于最大生效范围",
              type: "warning"
            });
            return ;
          }
