...
<template>
  <div class="BreakSet">
    ...
    <div v-if="EditVisible" class="indexContent">
      <BreakSetEdit ref="BreakSetEdit" :formType="formType"></BreakSetEdit>
    </div>

    <div v-else class="indexContent">
      <div class="toolbar">
        <el-form :inline="true" size="mini" class="demo-form-inline" @submit.native.prevent>
            <el-form-item label="违约金名称：" class="width-200">
              <el-input oninput ="value=value.replace(/\ +/g,'').replace(/[\r\n]/g,'')"  v-model="tableQuery.strategyName" @keyup.enter.native="search" clearable placeholder="违约金名称"></el-input>
            </el-form-item>
      ...
      <div class="kl-table" :style="{height: maxHeight + 'px'}">
        <el-table stripe border :data="tableData.list" v-if="tableShow" :max-height="maxHeight" class="wuserInfo-table">
          ...
          <el-table-column prop="minCalculateMoney" min-width="150" label="最低计算金额（元）" align="right">
          </el-table-column>

          <el-table-column prop="minPenaltyMoney" min-width="150" label="最低违约金额（元）" align="right">
          </el-table-column>

          <el-table-column prop="maxPenaltyMoney" min-width="150" label="最高违约金额（元）" align="right">
          </el-table-column>

          <el-table-column prop="dailyScaling" min-width="150" label="每天收取比例（%）" align="right">
          </el-table-column>

          <el-table-column prop="maxScaling" min-width="150" label="最高收取比例（%）" align="right">
          </el-table-column>

          <el-table-column prop="decimalPlacse" min-width="150" label="保留小数位数（%）" align="right">
          </el-table-column>
...
export default {
  ...
  mounted() {
    ...
    this.$nextTick(() => {
      ...
      this.common.changeTable(this, ".BreakSet .indexContent", [
        ".BreakSet .block",
        ".BreakSet .toolbar"
      ]);
    });
  ...
  methods: {
    ...
    add(val) {
      ...
      if (val === "add") {
        this.formType= 'PenaltyStrategyAdd'; 
        ...
        this.EditVisible = true;
        let _this = this;
        this.formType= 'PenaltyStrategyEdit'; 
        let params = {
    ...
    submitForm(formName) {
      ...
      this.$refs.BreakSetEdit.submitForm(formName, this.formType);
    }
  },
  watch: {
    maxHeight() {
