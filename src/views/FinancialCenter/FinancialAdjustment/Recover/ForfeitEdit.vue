<template>
  <div class="ForFeitEdit">
    ...
    >
      ...
      <el-form-item label="罚款类型：" :class="{'disable-item': NotDisabled}" prop="penaltyType">
        <el-select v-if="!NotDisabled" clearable v-model="formData.penaltyType" placeholder @change="selectChanged">
            <el-option label="按水量" value="1"></el-option>
            <el-option label="按金额" value="2"></el-option>
        </el-select>
        <span v-else>{{formData.penaltyTypeName}}</span>
      </el-form-item>
      ...
      <el-form-item  label="更新年水量：" :class="{'disable-item': NotDisabled  }" prop="updateYearWaterFlag" >
        <el-select v-if="!NotDisabled" clearable v-model="formData.updateYearWaterFlag" placeholder :disabled="!penaltyTypeFlag">
          <el-option
      ...
      <el-form-item  label="更新抄码：" :class="{'disable-item': NotDisabled }" prop="updateReadingNumFlag">
        <el-select v-if="!NotDisabled" clearable v-model="formData.updateReadingNumFlag" placeholder :disabled="!penaltyTypeFlag">
          <el-option
            v-for="(item,index) in updateReadingNumFlagOption"
            :key="index"
            :label="item.label"
      ...
      <el-form-item label="罚款水量：" :class="{'disable-item': NotDisabled  }" prop="currendPeriodWater">
        <el-input v-model="formData.currendPeriodWater" v-if="!NotDisabled" placeholder @input="changeAddWater" :disabled="!penaltyTypeFlag"
        onkeyup="if(this.value.length==1){this.value=this.value.replace(/[^0-9]/g,'')}else{this.value=this.value.replace(/\D/g,'')}" 
        onafterpaste="if(this.value.length==1){this.value=this.value.replace(/[^0-9]/g,'')}else{this.value=this.value.replace(/\D/g,'')}"></el-input>
      ...
      <el-form-item :class="{'disable-item': NotDisabled}" v-if="penaltyTypeFlag">
      <el-checkbox-group v-model="formData.notCalLadder" >
                    <el-checkbox label="1" >不计阶梯</el-checkbox>
                </el-checkbox-group>
      </el-form-item>
       <el-form-item class="disable-button" >
        <el-button v-if="!NotDisabled && penaltyTypeFlag" @click="calCulateWater()" class="calculate">计算</el-button>
      </el-form-item> 

       <el-table v-if="!penaltyTypeFlag" ref="elTable" stripe border :data="tableData" class="change-tables-table" 
          @selection-change="getSelectData"  :summary-method="getSummaries" show-summary>
        ...
        <el-table-column type="index" label="NO." width="50" fixed="left" >
          ...
          <el-table-column  prop="money" min-width="100" label="金额">
             <template slot-scope="scope">             
              <el-input oninput ="value=value.replace(/\ +/g,'').replace(/[\r\n]/g,'')"  size="mini" :disabled="NotDisabled" v-model="scope.row.money"  ></el-input>
            </template>
          </el-table-column>
        </el-table>

     
      <el-table v-else ref="elTable" stripe border :data="tableData" class="change-tables-table" @selection-change="getSelectData"  :summary-method="getSummaries" show-summary>
...
export default {
  ...
  methods: {
    ...
    selectChanged(value){
        ...
        _this.tableData = []
        if(value == '1') {
          _this.penaltyTypeFlag = true
        }
        else {
           _this.penaltyTypeFlag = false
           _this.formData.currendPeriodWater = 0;
           _this.formData.updateReadingNumFlag = 0;
           _this.formData.updateYearWaterFlag = 0;
           _this.formData.currendPeriodNum = _this.formData.priorPeriodNum
            ...
            .then(res => {
              if(res.length && res.length > 0) {
                res.forEach(t=>{
                    let cost = {}
                    _this.$set(cost,'costNo',t.costNo)
                    _this.$set(cost,'costName',t.costName)
                    _this.$set(cost,'money','')
                    this.tableData.push(cost);
                })
                // 获取完数据调用
    ...
    init() {
      ...
      this.$api
        ...
        .then(res => {
          ...
          if(this.formData.penaltyType == '2')
            this.penaltyTypeFlag = false;
          else
            this.penaltyTypeFlag = true;
    ...
    calculate(priceDetail){
      ...
      let notCalLadder = 0;
      if(_this.formData.notCalLadder.length != null && _this.formData.notCalLadder.length>0)
          notCalLadder = 1;
  ...
  changePriorNum(val){
    ...
    getFormbData() {
      ...
      if(_this.formData.notCalLadder != null && _this.formData.notCalLadder.length > 0)
        _this.formData.notCalLadder = _this.formData.notCalLadder.toString()
      else
        _this.formData.notCalLadder = null;
    ...
    submit() {
      let params = {};
      if(this.formData.currendPeriodWater<0 && this.formData.penaltyType == '1'){
        this.$notify({
          title: "警告",
          message: "罚款水量不能小于0",
          type: "warning"
