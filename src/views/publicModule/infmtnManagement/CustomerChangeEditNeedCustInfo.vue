<template>
  <div class="custInfo">
    ...
    <el-form :model="formData" class="formBill" size="mini" :rules="rules" ref="ruleForm">
      ...
      <el-form-item label="客户地址：" prop="clientAddr" :class="{'disable-item':editType==0, 'two-input': true}">
        ...
        <template v-else>
            ...
            <el-input oninput ="value=value.replace(/\ +/g,'').replace(/[\r\n]/g,'')"  v-model="formData.clientAddrDetail" placeholder="" clearable>
              <el-button slot="append" type="primary" size="mini" @click="copyAddr">粘贴</el-button>
            </el-input>
             
         <el-tooltip  effect="dark" content="复制到装表地址" placement="top-start">
         <i class="el-icon-warning-outline" @click="copyAddr" ></i>
       </el-tooltip>
          </template>
      ...
      <el-form-item label="证件号码：" prop="certNo" v-if="!disableCertNo" style="width: calc(46% + 10px)" :class="{'disable-item':editType==0}">
        <span v-if="editType==0">{{formData.certNo}}</span>
        <el-input oninput ="value=value.replace(/\ +/g,'').replace(/[\r\n]/g,'')"  v-else v-model="formData.certNo" placeholder="" clearable>
      ...
      <el-form-item label="手机号码：" prop="mobile" :class="{'disable-item':editType==0}">
...
export default {
  name: "custInfo",
  props: ['editType', 'wayOpen', 'disableCertNo'],
  components: {
    addrAdd
  },
  data() {
    ...
    return {
      ...
      rules: {
        ...
        clientAddr:[
            ...
            { required: true, validator: checkAddr, trigger: 'blur'}
            ],
     ...
     beforeDestroy() {
    eventBus.$off('copyUserAddr')
  },
  mounted() {
    if (this.editType != 0) {
      this.getDictionarySelect()
    }
     eventBus.$on('copyUserAddr', (res) => {
      this.formData.clientAddrDetail = res.clientContractAddrDetail
      this.formData.clientAddr = res.clientContractAddr
      this.formData.ctmAddr = res.setupMeterAddr
      this.adminAreaData = res.adminData
    })
  },
  ...
  methods: {
   ...
   copyAddr(){
    ...
    if(this.formData.clientAddr || this.formData.clientAddrDetail){
       eventBus.$emit("copyClientAddr",copyData);
          this.$notify({
          title: '提示',
          message: '已成功复制到装表地址',
          type: 'success'
   ...
   getCtmData(item){
    this.adminAreaData.forEach(e=>{
      if(e.id == item){
       this.copyCtmName = e.fullName
      }
    })
   },
