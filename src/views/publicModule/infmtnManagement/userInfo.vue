...
export default {
  ...
  data () {
    return {
      isShowCapacityWaterPrice: false,
      openWay: 'userInfo',
      editFormData: {},
      labelNames: "",//标题名称
      labelStyle: false,
      ...
      remenberContractNo: "",//保存没有修改前合同编号的内容
    };
  },
  beforeDestroy () {
  ...
  mounted () {
    this.getFormPage()
    eventBus.$on('copyClientAddr', (res) => {
      this.$refs.dynamicForm.editFormData.clientContractAddrDetail = res.clientAddrDetail
      this.$refs.dynamicForm.editFormData.clientContractAddr = res.clientAddr
      this.$refs.dynamicForm.editFormData.clientContractAddrId = res.clientAddr
      this.$refs.dynamicForm.adminAreaData = res.adminData
    })
  ...
  methods: {
    ...
    init (val) {
      if (val) {
        this.formData = val;
        this.remenberContractNo = JSON.parse(JSON.stringify(this.formData.contractNo));
        if (val.clientContractAddr && val.clientContractAddr instanceof Array && val.clientContractAddr.length === 1)
          this.formData.clientContractAddr = val.clientContractAddr[0]
        // this.selectBAreaOptions(this.formData.businessAbode);
        this.getFormPage()
        eventBus.$emit('adminArea', this.formData.clientContractAddr)
      }else{
        this.remenberContractNo = "";
      }
    },
    ...
    getCapaMoney (meterBore, useWaterType) {//需要口径、用水类型
      ...
      let params = {
        ...
        data: {
          meterBore: meterBore,
          useWaterType: this.common.handleTreeData(useWaterType),
        }
      };
      this.$api
        .fetch({
        ...
        .then(res => {
          this.$refs.dynamicForm.setFormItem('capaMoney', res.capaMoney)
          this.formData = this.$refs.dynamicForm.editFormData; // 获取已填写的信息，防止重新init丢失已填数据
          this.formData.capaMoney = res.capaMoney;
          this.getFormDetail(this.formData); //更新动态表单的容量水价
        })
    ...
    changeArea (val) {
      //console.log("val",val);
      eventBus.$emit('PjMeterUpdateBusinessArea',val)
