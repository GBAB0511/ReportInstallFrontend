          ...
          <div class="kl-table">
            ...
            <el-table-column prop="factoryNo" min-width="100" label="表身码">
              ...
              <el-table-column prop="meterFactory" min-width="100" label="水表厂家">
                <template slot-scope="scope">
                  <el-select clearable v-model="scope.row.meterFactory" placeholder="水表厂家" @change="(value)=>getMeterModel(scope.row)">
              ...
              <el-table-column prop="meterModel" min-width="120" label="水表型号">
                <template slot-scope="scope">
                  <el-select clearable v-model="scope.row.meterModel" placeholder="水表型号" @change="(value)=>getWaterMeterData(value,scope.row)">
                    <el-option v-for="(item,index) in scope.row.meterModelData" :key="index" :label="item.name" :value="item.value"></el-option>
                  </el-select>
                </template>
              </el-table-column>
              ...
              <el-table-column prop="sim" min-width="100" label="SIM卡">
                <template slot-scope="scope">
                  <el-input oninput ="value=value.replace(/\ +/g,'').replace(/[\r\n]/g,'')"  v-model="scope.row.sim" size="mini"></el-input>
                </template>
              </el-table-column>
             ...
             <el-table-column prop="imsi" min-width="100" label="imsi号">
              ...
              <el-table-column prop="factoryName" min-width="100" label="水表厂家">
              </el-table-column>
              <el-table-column prop="meterTypeName" min-width="100" label="水表分类">
              </el-table-column>
              <el-table-column prop="valveControlName" min-width="120" label="是否阀控">
              </el-table-column>
              <el-table-column prop="protocolName" min-width="100" label="下行规约">
              </el-table-column>

              <el-table-column prop="meterFormName" min-width="100" label="水表型式">
              </el-table-column>
              <el-table-column prop="meterSensorName" min-width="100" label="水表传感器">
              </el-table-column>
  ...
  methods: {
    ...
    getBillData(res) {
      ...
      this.$refs.billInfo.init(this.billData);
       ...
       // 获取水表信息
     async getWaterMeterData (val,row) {
      try {
            // 水表型号查出的信息
    ...
    getDictionary() {
      ...
      var params = {
        ...
        "data": "MMT,MMS,MMF,TPW"//水表制式、水表状态、新旧表
      }
      this.$api.fetch({
        params: params,//参数
      }).then(res => {
    ...
    getMeterModel(row){
      ...
      this.$api.fetch({
        ...
        console.log('meterModelData', row.meterModelData)
        // row.meterModel=res[0].value
      })
    },
    // 水表型号下拉数据(回显)
    getMeterModel1(item){
      let factoryId = item.meterFactory
      var _this = this
      var params = {
        "busicode": "WaterMeModelOpList",
        "data": {
    ...
    submit() {
      if (this.activeName == '1') {
        ...
        if( this.$refs.SelectWaterTable.selectData &&  this.$refs.SelectWaterTable.selectData.length>0) {
          this.$refs.SelectWaterTable.selectData.forEach(item => {

            if(this.deleteTabelDate && this.deleteTabelDate.length > 0) {
              var exists = 0;
              this.deleteTabelDate.forEach(y => {
                if(item.meterNo == y.meterNo)
                    exists = 1;
              })
              if(exists == 0)
                this.deleteTabelDate.push(item);
            }else
              this.deleteTabelDate.push(item);
          })
        }
    ...
    async getMeterData(data) {
      if(data && data.length > 0) {
        ...
        data.forEach(async item=>{
          if(this.updateTabelDate && this.updateTabelDate.length > 0) {
            var exists = 0
            this.updateTabelDate.forEach(y=>{
                if(y.meterNo == item.meterNo)
                  exists = 1
            })
            if(exists == 0) {
                try {
                  ...
                  if(item.meterModel){
                    ...
                    // if (tableData) {
                    //   tableData.meterModelData = []
                    // }
                    this.updateTabelDate.push(tableData)
                  }else{
                    let tableData = Object.assign({}, item, {})
                    this.updateTabelDate.push(tableData)
                  }
    ...
    save() {
      this.$refs['ruleForms'].validate((valid) => {
        if (valid) {
          ...
          console.log(this.editNeedData.busicode+"1111");
