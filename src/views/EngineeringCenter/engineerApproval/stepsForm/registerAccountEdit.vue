<template>
  <div class="registerAccountEdit">
    <el-form :model="formData" class="formBill" :inline="true" :disabled="NotDisabled" ref="ruleForm" size="mini" label-width="130px">
      <!-- 客户信息 -->
      <custInfo ref="custInfo" v-bind="$attrs" :editType="custEditType" :wayOpen="wayOpen" :disableChangeCertNo="disableChangeCertNo" ></custInfo>
      <!-- 账户信息 -->
      <accountInfo ref="accountInfo" v-bind="$attrs" :editType="accountEditType" :wayOpen="wayOpen" :disableAccountNo="disableAccountNo"></accountInfo>
      <!-- 开票信息 -->
      <invoiceInfo ref="invoiceInfo" v-bind="$attrs" :editType="invoiceEditType" :wayOpen="wayOpen"></invoiceInfo>
      <!-- 用户信息 -->
      <userInfo ref="userInfo" v-bind="$attrs" :addWay="NotDisabled" :editType="userInfoEditType" :wayOpen="wayOpen"></userInfo>
       <!-- 用户-IC卡信息 -->
      <ICCardInfo ref="ICCardInfo" v-bind="$attrs" :addWay="NotDisabled" :editType="userInfoEditType" :wayOpen="wayOpen"></ICCardInfo>
      <!-- 抄表信息 -->
      <meterReadInfo  ref="meterReadInfo" v-bind="$attrs" :editType="meterEditType" :wayOpen="wayOpen"></meterReadInfo>
      <!-- 水表信息 -->
      <waterMInfo ref="waterMInfo" v-bind="$attrs" :addWay="NotDisabled" :editType="waterMEditType" :wayOpen="wayOpen"></waterMInfo>
      <!-- 价格信息 -->
      <priceInfo v-if="priceInfoShow" v-bind="$attrs" ref="priceInfo" :idCuster="idCuster" :editType="priceEditType" :wayOpen="wayOpen"></priceInfo>
...
export default {
  name: "registerAccountEdit",
  props: ["idCuster", 'editData', 'NotDisabled','listIds','singleCustomerFlag'],
  components: {
    uploadFile,
    billInfo,
    custInfo,
  ...
  mounted() {
    ...
    this.priceInfoShow = true
     ...
     this.accountInfoId = this.editData.accountInfo.id;
     this.ctmInfoId = this.editData.ctmInfo.id;
    ...
    if (this.idCuster) {
      ...
      //this.getHtmlData()
      this.$refs.meterReadInfo.AreaInit()
      let pjInfoquery = this.editData;
      this.getCustData(pjInfoquery);
      this.getAccountData(pjInfoquery);
      this.getInvoiceData(pjInfoquery);
    ...
    eventBus.$on('getCapaMoney', ()=>{ //设置容量水价事件
     ...
     eventBus.$on('addrSelectChange', (id)=>{ //订阅更新水表区域事件
        //console.log("id",id);
         let params = {
        busicode: "PjMeterUpdateArea",

        data: { areaId: id, pjMeterIdList: this.listIds }
      }
      if(this.singleCustomerFlag!=undefined&&this.singleCustomerFlag!=0){
        this.$api.fetch({ params }).then(res => {
        // console.log("11111");
        });
      }
    });
    eventBus.$on('PjMeterUpdateBusinessArea',item=>{
      //console.log(item);
      this.businessArea=item
      let params = {
        busicode: "PjMeterUpdateBusinessArea",
        data: { businessArea: this.businessArea, pjMeterIdList: this.listIds }
      }
      if(this.singleCustomerFlag!=undefined&&this.singleCustomerFlag!=0){
        this.$api.fetch({ params }).then(res => {
          eventBus.$emit("initBusinessArea",1)
        });
      }
    })
  },
  beforeDestroy() {
    ...
    eventBus.$off('addrSelectChange')
    eventBus.$off('PjMeterUpdateBusinessArea')
  },
  watch: {},
  methods: {
    init() {
      ...
      let discountEdit = false;
      let params = {
        busicode: "PjMeterDetail",
        data: { id: this.idCuster }
      };
      this.$api.fetch({ params })
        .then(res => {
          this.formData = res;
          this.$refs.meterReadInfo.changeArea(res.userInfo.businessArea);
          this.getFormData(res);
          this.userInfoId = res.userInfo.id;
    ...
    getCustData(res) {
      ...
      this.custData = {
        ...
        clientAddrName:res.ctmInfo.clientAddrName,
    ...
    getUserData(res) {
      ...
      this.userData = {
        ...
        userType:res.userInfo.userType,//用户特殊类型
        lastReadingNum:res.userInfo.lastReadingNum,//最后抄码
        dmaArea: res.userInfo.dmaArea, //DMA分区
        dmaAreaName: res.userInfo.dmaAreaName, //DMA分区
      };
    },
    ...
    getWaterMData(res) {
      let valMeter = {
        ...
        meterPerformanceType:res.userInfo.meterPerformanceType,//水表绩效类型
        meterPerformanceTypeName:res.userInfo.meterPerformanceTypeName,//水表绩效类型
      };
      let key = "userInfo";
    ...
    getWMeterData(res) {
      ...
      let valMeter = {
        ...
        meterBjms:res.userInfo.meterBjms,
   ...
   //客户信息
    ...
    //console.log("-----------");
       ...
       let msInfoData=localStorage.getItem('RegisterAccountMsInfoList')
       //console.log(this.$refs.userInfo.$refs.dynamicForm.editFormData);
      let tempAccountInfoId;
      let tempCtmInfoId;
      if(this.formData.accountInfo && this.formData.accountInfo.id != this.accountInfoId){
        tempAccountInfoId = this.formData.accountInfo.id;
      }else{
        tempAccountInfoId = this.accountInfoId
      }
      if(this.formData.ctmInfo && this.formData.ctmInfo.id != this.ctmInfoId){
        tempCtmInfoId = this.formData.ctmInfo.id;
      }else{
        tempCtmInfoId = this.ctmInfoId;
      }
      let val = {
        consultId:this.idCuster?undefined:this.editData.consultId,
        accountInfo: {
          id: tempAccountInfoId,
          ...this.$refs.accountInfo.formData,
          ...this.$refs.invoiceInfo.formData
        },
        ctmInfo: {
        ...
        userInfo: {
          id:this.idCuster?this.userInfoId:undefined,
         ...this.$refs.waterMInfo.$refs.dynamicForm.editFormData,
         ...this.$refs.ICCardInfo.$refs.dynamicForm.editFormData,
         useTimeLimit:this.$refs.userInfo.$refs.dynamicForm.editFormData.useTimeLimit!=''?this.$refs.userInfo.$refs.dynamicForm.editFormData.useTimeLimit:undefined,
          // ...this.$refs.waterMInfo.formData.userInfo,
          ...this.$refs.meterReadInfo.formData,
          ...this.$refs.priceInfo.formData,
          ...this.$refs.userInfo.$refs.dynamicForm.editFormData,
        ...
        id:this.idCuster!=undefined?this.idCuster:undefined
      };

      let msData=JSON.parse(msInfoData)
      console.log(msData);
      let msInfo={}
      if(msInfoData==undefined){
       val['msInfo']=undefined
      }else{
        for(let i=0;i<msData.list.length;i++){
          if(val.userInfo.meterNo==msData.list[i].meterNo){
            msInfo=msData.list[i]
          }
        }
        console.log(msInfo);
      ...
      this.businessArea=val.userInfo.businessArea
      // dma分区 只需要传最后一个值过去
      val.userInfo.dmaArea = this.common.handleTreeData(
        val.userInfo.dmaArea
      );
      //数据格式处理
